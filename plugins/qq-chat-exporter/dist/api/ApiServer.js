...
export class QQChatExporterApiServer {
    ...
    setupRoutes() {
        ...
        this.app.post('/auth', (req, res) => {
            ...
            const clientIP = req.ip || req.connection.remoteAddress || '';
            if (!token) {
                return this.sendErrorResponse(res, new SystemError(ErrorType.VALIDATION_ERROR, '缺少访问令牌', 'MISSING_TOKEN'), req.requestId, 400);
            }
            ...
            if (isValid) {
                ...
                return this.sendErrorResponse(res, new SystemError(ErrorType.AUTH_ERROR, '无效的访问令牌', 'INVALID_TOKEN'), req.requestId, 403);
            }
        });
        // 更新服务器地址配置
        this.app.post('/api/server/host', async (req, res) => {
            try {
                ...
                if (!host || typeof host !== 'string') {
                    return this.sendErrorResponse(res, new SystemError(ErrorType.VALIDATION_ERROR, '服务器地址不能为空', 'INVALID_HOST'), req.requestId, 400);
                }
                ...
                return this.sendErrorResponse(res, new SystemError(ErrorType.CONFIG_ERROR, '更新服务器地址失败', 'UPDATE_HOST_FAILED'), req.requestId);
            }
        });
        // 系统信息
        ...
        this.app.get('/api/groups/:groupCode', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, groupDetail, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.get('/api/groups/:groupCode/members', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, members, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.get('/api/friends/:uid', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, friendDetail, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.get('/api/users/:uid', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, userInfo, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.get('/api/tasks', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, { tasks }, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.delete('/api/tasks/:taskId', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, { message: '任务已彻底删除' }, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.post('/api/sticker-packs/export', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, result, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.post('/api/scheduled-exports', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, scheduledExport, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.get('/api/scheduled-exports', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, { scheduledExports }, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.put('/api/scheduled-exports/:id', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, updatedTask, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.delete('/api/scheduled-exports/:id', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, { message: '定时导出任务已删除' }, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.get('/api/scheduled-exports/:id/history', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, { history }, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.get('/api/exports/files', async (req, res) => {
            try {
                ...
                this.sendSuccessResponse(res, { files: exportFiles }, req.requestId);
            }
            catch (error) {
                this.sendErrorResponse(res, error, req.requestId);
            }
        ...
        this.app.get('/api/exports/files/:fileName/resources/*', (req, res) => {
            try {
                ...
                const resourcePath = req.params[0]; // 例如: images/xxx.jpg
                // 安全检查：防止路径遍历攻击
        ...
        this.app.use((req, res) => {
            this.sendErrorResponse(res, new SystemError(ErrorType.API_ERROR, `API端点不存在: ${req.method} ${req.path}`, 'ENDPOINT_NOT_FOUND'), req.requestId, 404);
        });
        // 错误处理中间件
    ...
    async processExportTaskAsync(taskId, peer, format, filter, options, fileName, downloadUrl) {
        ...
        try {
            ...
            const chatInfo = {
                name: chatName,
                type: (peer.chatType === ChatType.KCHATTYPEGROUP ? 'group' : 'private')
            };
     ...
     * 发送错误响应
     */
    sendErrorResponse(res, error, requestId, statusCode = 500) {
        let systemError;
        if (error instanceof SystemError) {
            systemError = error;
        }
        else {
            systemError = new SystemError(ErrorType.UNKNOWN_ERROR, error.message || '未知错误', 'UNKNOWN_ERROR');
        }
        const response = {
            success: false,
            error: {
    ...
    async saveTaskToDatabase(task) {
        try {
            const config = {
                ...
                chatAvatar: '', // 可以后续添加
                formats: [task.format?.toUpperCase() || 'JSON'],
                filter: {
                    startTime: task.filter?.startTime,
                    endTime: task.filter?.endTime,
                            ...
                            if (!fileInfo.displayName) {
                                try {
                                    if (fileInfo.chatType === 'friend') {
                                        const friends = await this.core.apis.FriendApi.getBuddy();
                                        const friend = friends.find((f) => f.coreInfo?.uid === fileInfo.chatId);
     ...
     * 从HTML内容中提取聊天信息
     */
    extractChatInfoFromHtml(htmlContent) {
        const info = {};
        try {
